...
def main():
    ...
    if opcion == "Información":
        ...
        st.write("Los tipos de receta son: Acompañamiento, Cena, Cumpleaños, Desayuno, Entrante, Merienda, Plato principal y Postre.")
        seleccion_difi = st.text_input("Introduce la dificultad de la receta:")
        st.write("Las dificultades de las recetas son: muy bajo, bajo, medio, alto y muy alto")

        if st.button("Buscar"):
            encuentra = recetas.elegir_receta(seleccion_tipo, seleccion_difi)
            if encuentra.empty:
